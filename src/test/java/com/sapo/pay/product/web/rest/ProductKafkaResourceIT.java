...
import com.sapo.pay.product.config.KafkaProperties;
...
class ProductKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/product-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/product-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
